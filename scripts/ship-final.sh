#!/usr/bin/env bash
# Final 3 batches: p16-2, p16-3, p16-4.
# p16-1, p17, p18 already shipped on main (both repos).
set -euo pipefail
...
grove_www_sync_before_batch() {
  ...
  [[ "$branch" != "main" ]] && grove_www_sync_after_batch "$branch"
  ...
  if ! git -C "$GROVE_WWW" diff --quiet || ! git -C "$GROVE_WWW" diff --cached --quiet; then
    log "ERROR: grove-www dirty"; git -C "$GROVE_WWW" status --short; exit 1
  fi
...
grove_www_sync_after_batch() {
  ...
  if [[ "$ahead" -eq 0 ]]; then
    log "grove-www: on $branch, no commits — checkout main"
    git -C "$GROVE_WWW" checkout main --quiet 2>/dev/null || true
    return
  fi

  log "grove-www: $branch has $ahead commit(s) — consolidating onto main"
...
run_wave() {
  ...
  if [[ "${sha}" == "${pre_sha}" ]]; then
    log "NOTE: grove HEAD unchanged — work may be purely in grove-www."
  else
...
if ! git diff --quiet || ! git diff --cached --quiet; then
  log "ERROR: grove dirty"; git status --short; exit 1
fi
[[ "$(git rev-parse --abbrev-ref HEAD)" != "main" ]] && { log "ERROR: grove not on main"; exit 1; }
git fetch origin main --quiet
[[ "$(git rev-parse main)" != "$(git rev-parse origin/main)" ]] && { log "ERROR: grove not synced"; exit 1; }
...
log ""
log "✅ DONE — final 3 batches in $(( ELAPSED / 60 )) min."
